...
export function getNarrativeArcDirective(stepNum: number): string {
  if (stepNum >= TOTAL_STEPS) {
    return `NARRATIVE ARC PHASE — THE GRAND EPILOGUE & DEFINITIVE RESOLUTION (Step ${TOTAL_STEPS} — THE END): Step ${TOTAL_STEPS} is the ABSOLUTE, PERMANENT, AND DEFINITIVE FINAL SCENE of the film. The central conflict and antagonist have already been defeated. This scene delivers the emotional and philosophical resolution, closure for the surviving characters, and an iconic, unforgettable final cinematic visual (fading into the dawn or stars). Absolutely DO NOT introduce any new threats, villains, conflicts, or cliffhangers. This is the definitive THE END.`;
  }
  if (stepNum === TOTAL_STEPS - 1) {
    return `NARRATIVE ARC PHASE — THE ULTIMATE CLIMAX & FINAL SHOWDOWN (Step ${TOTAL_STEPS - 1} / 49): This is the PEAK CLIMAX of the entire story! The grand showdown with the main antagonist or final catastrophic threat reaches its explosive, irreversible resolution. The core mystery is completely unlocked, the villain's master plan is broken, and the protagonist makes their defining, heroic stand. This scene must definitively settle the central conflict so that Step ${TOTAL_STEPS} serves as the peaceful, emotional epilogue. The two voting options (A vs B) determine the exact manner and legacy of this final victory.`;
  }
  if (stepNum >= 40) {
    return `NARRATIVE ARC PHASE — DENOUEMENT / CONVERGENCE (steps 40-48, building toward the final showdown): The story is in its closing act. Conflicts converge rapidly: alliances are solidified, the antagonist's ultimate fortress/device is breached, and stakes become personal and monumental. Converge every loose thread toward the climax at step 49 and the definitive ending at step ${TOTAL_STEPS}.`;
  }
  if (stepNum >= 11) {
